...
//modify by luoran for single mode(start)

//modify by luoran for single mode(end)
public class InfoDropTarget extends ButtonDropTarget {
    ...
    //add by luoran for single mode(start)
    @Override
    public boolean isDropEnabled() {
        return !LauncherAppState.isDisableAllApps();
    }

    //add by luoran for single mode(end)
    public static boolean supportsDrop(Context context, Object info) {
        if (LauncherAppState.isDisableAllApps()) {
            ...
            if (info instanceof ShortcutInfo) {
                ...
                isSystemApp = Utilities.isSystemApp(context, shortcutInfo.intent);
                int shortcutItemType = shortcutInfo.itemType;
                if (shortcutItemType != 0) {
                    isSystemApp = false;
                }
            } else if (info instanceof LauncherAppWidgetInfo) {
                return false;
            }
            return info instanceof AppInfo || isSystemApp;
        } else {
